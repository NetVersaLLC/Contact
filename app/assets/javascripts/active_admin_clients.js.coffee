...
window.loadPayloads = ()->
  $.getJSON '/payload_nodes/'+$('#payload_categories_select').val()+'.json', (data)->
    ...
    $.each data, (i,e)->
      console.log e 
      html =  "<li data-payload-id='#{e.id}' data-payload-name='#{e.payload_name}' >"
      html += e.payload_name
    ...
    $('#payload_list_container ul, #payload_trash').sortable({connectWith: '#payload_list_container ul, .connect_thru'})
    ...
    $('#save_payload_nodes').click (e) -> 
      payload = {} 
      payload.site = $('#payload_categories_select').val()
      payload.tree = [] 
      payload.trash = []
