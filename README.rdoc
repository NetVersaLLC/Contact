...
Exceptions can be thrown at any point in the code and they will be automatically caught and reported back to the server. However, execptions are for unrecoverable errors only. They are not to be used for normal process control. If the job is uanble to complete it is best to call ContactJob.failure(job, "Message"). This will notify the server of the failure.
...
To do this you can run 'rails console' then call User.first.authentication_token to get the autentication token. Next you need the business id, you can get this with 'Business.first.id'. It will be "1" if you have just installed a new system.
...
Install required RubyGems:
    ...
    bundle install --without default assets production
