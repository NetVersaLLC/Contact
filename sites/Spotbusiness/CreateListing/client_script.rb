def add_listing(data)
  ...
  @browser.select_list(:name => 'cb_businesscategory').select data[ 'category' ]
...
enter_captcha( data )
  ...
  if @browser.text.include?(@confirmation_msg)
    ...
    RestClient.post "#{@host}/accounts.json?auth_token=#{@key}&business_id=#{@bid}", 'account[email]' => data[ 'username' ], 'account[password]' => data['password'], 'model' => 'Spotbusiness'
  else
